...
import { Bold, Italic, Link as LinkIcon, List, ListOrdered, Quote, Code, Undo, Redo } from 'lucide-react';
...
const ToolbarBtn = ({ onClick, isActive, icon: Icon }) => (
  <button
    ...
    className={`p-2 rounded-sm transition-colors ${isActive ? 'bg-red-600 text-white' : 'hover:bg-white/10 text-zinc-500 hover:text-white'}`}
  >
    <Icon size={14} />
  </button>
);
...
export default function AnnouncementEditor({ content, onChange, placeholder = "Start typing..." }) {
  const editor = useEditor({
    ...
    editorProps: {
        attributes: {
            class: 'prose prose-sm dark:prose-invert max-w-none focus:outline-none min-h-[300px] p-4 font-light leading-relaxed font-sans text-zinc-200',
        },
    },
  });
  ...
  return (
    <div className="border border-white/10 bg-black flex flex-col focus-within:ring-1 focus-within:ring-red-600 transition-all">
