...
export default function FeedModal({ item, isOpen, onClose, initialIndex = 0 }) {
  ...
  const fetchComments = async () => {
      ...
      if (isChangelog) {
          query = query.eq('changelog_id', item.id);
      } else {
          query = query.eq('project_id', item.id).is('changelog_id', null);
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl w-[95vw] h-[90vh] p-0 gap-0 bg-black border-border overflow-hidden flex flex-col md:flex-row z-[9999]">
        ...
        <div className="flex-1 bg-black relative flex items-center justify-center border-b md:border-b-0 md:border-r border-border min-h-[40vh] md:min-h-full">
            ...
            {item.media.length > 1 && (
                <>
                    ...
                    <button onClick={() => setIndex((prev) => (prev + 1) % item.media.length)} className="absolute right-4 z-40 p-2 bg-black/50 text-white hover:bg-white/10 rounded-full">
        ...
        {/* RIGHT: Context & Interaction */}
        <div className="w-full md:w-[400px] flex flex-col bg-background h-full border-l border-border">
            
            {/* Header */}
            <div className="p-4 border-b border-border flex items-start gap-3 bg-secondary/5">
                ...
                <div className="space-y-6">
                        ...
                        <div className="text-sm leading-relaxed font-light text-foreground/90">
                            ...
                            {isLongDesc && (
                                <button 
                                    ...
                                    className="text-muted-foreground hover:text-foreground mt-2 font-mono text-xs hover:underline block"
                                >
                    ...
                    {(demoLink || sourceLink || videoLink) && (
                        <div className="flex flex-wrap gap-2 pt-2">
                            {demoLink && (
                                <a href={demoLink} target="_blank" className="flex items-center gap-1.5 px-3 py-1.5 bg-secondary/10 border border-border hover:border-accent hover:text-accent transition-colors text-[10px] font-mono uppercase">
                                    <Globe size={12} /> Live Demo
                                </a>
                            )}
                            {sourceLink && (
                                <a href={sourceLink} target="_blank" className="flex items-center gap-1.5 px-3 py-1.5 bg-secondary/10 border border-border hover:border-foreground hover:text-foreground transition-colors text-[10px] font-mono uppercase">
                                    <Github size={12} /> Source
                                </a>
                            )}
                            {videoLink && (
                                <a href={videoLink} target="_blank" className="flex items-center gap-1.5 px-3 py-1.5 bg-secondary/10 border border-border hover:border-red-500 hover:text-red-500 transition-colors text-[10px] font-mono uppercase">
                                    <Play size={12} /> Video
                                </a>
                            )}
                        </div>
                    ...
                    {item.tech && item.tech.length > 0 && (
                        <div className="flex flex-wrap gap-2 pb-4 border-b border-border/50">
                            {item.tech.map((t, i) => (
                                <span key={i} className="px-2 py-1 bg-secondary border border-border text-[10px] font-mono text-muted-foreground">
                                    {t}
                                </span>
                            ))}
                        </div>
                    ...
                    <div>
                        ...
                        {loadingComments ? (
                            ...
                            <div className="text-center py-8 text-xs text-muted-foreground border border-dashed border-border opacity-50">
                                No signals detected.
                            </div>
                        )}
                    </div>
            ...
            <div className="p-4 border-t border-border bg-background">
                <div className="flex items-center gap-4 mb-3">
                    <button onClick={handleLike} className={`transition-colors ${isLiked ? 'text-red-500' : 'text-muted-foreground hover:text-foreground'}`}>
                ...
                <div className="relative mb-3">
                    <input 
                        ...
                        className="w-full bg-secondary/10 border border-border h-10 pl-3 pr-10 text-xs font-mono focus:border-accent outline-none"
                        onKeyDown={(e) => e.key === 'Enter' && handlePostComment()}
                    />
                    <button 
                        onClick={handlePostComment}
                        disabled={isSubmitting}
                        className="absolute right-2 top-1/2 -translate-y-1/2 text-accent disabled:opacity-50"
                    >
                ...
                <Link href={`/project/${isChangelog ? item.project.slug : item.slug}`}>
                    <Button className="w-full rounded-none font-mono text-xs uppercase h-10 bg-foreground text-background hover:bg-accent hover:text-white">
                        View Full Project
                    </Button>
                </Link>
            </div>
